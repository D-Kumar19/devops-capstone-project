...
class TestAccountService(TestCase):
    ...
    def test_read_an_account(self):
        """It should Create an Account and we should be able to retrieve it as well"""
    ...
    def test_account_not_found(self):
        """Reading an account that is not existing"""
    ...
    def test_delete_account_not_found(self):
        """Deleting an account that is not existing"""
        response = self.client.delete(f"{BASE_URL}/0")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_update_account(self):
        """It should update a created account"""
        account = self._create_accounts(1)[0]
        ...
        self.assertEqual(data["date_joined"], str(new_account.date_joined))

    def test_update_account_not_found(self):
        """Updating an account that is not existing"""
        new_account = AccountFactory()
        response = self.client.put(
            f"{BASE_URL}/0",
            json=new_account.serialize(),
    ...
    def test_flask_tailsman(self):
        """It will check if the Flask Tailsman is working"""
    ...
    def test_cors_policies(self):
        """It will check the CORS policies"""
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
